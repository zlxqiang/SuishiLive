...
public class MagicHudsonFilter extends GPUImageFilter {
	...
	protected void onInitialized(){
	    ...
	    runOnDraw(new Runnable(){
		    public void run(){
		    	inputTextureHandles[0] = OpenGlUtils.loadTexture(ContextInstance.getInstance(), "filter/hudsonbackground.png");
				inputTextureHandles[1] = OpenGlUtils.loadTexture(ContextInstance.getInstance(), "filter/overlaymap.png");
				inputTextureHandles[2] = OpenGlUtils.loadTexture(ContextInstance.getInstance(), "filter/hudsonmap.png");
		    }
	    });
	}
}
