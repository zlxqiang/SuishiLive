...
{ 
	...
	vec4 platform = mix(vec4(c.bg, K.wz), vec4(c.gb, K.xy), step(c.b, c.g));
	vec4 q = mix(vec4(platform.xyw, c.r), vec4(c.r, platform.yzx), step(platform.x, c.r));
...
vec3 hsv2rgb(vec3 c) { 
	...
	vec3 platform = abs(fract(c.xxx + K.xyz) * 6.0 - K.www);
	return c.z * mix(K.xxx, clamp(platform - K.xxx, 0.0, 1.0), c.y);
} 
